...
describe("OpenId3Account", function () {
  ...
  const buildOperator = (
    ...
    return hre.ethers.solidityPacked(
      ...
      [validAfter ?? 0, validUntil ?? 0, signer.address]
    );
  };
  ...
  it("should deploy account", async function () {
    ...
    const operator = buildOperator(deployer);
  ...
  it("should clone account with admin only", async function () {
    ...
    expect(await account.getMode()).to.eq(0);
    expect(await account.getAdmin()).to.eq(await admin.getAddress());
    expect(await account.getOperator()).to.eq(hre.ethers.ZeroHash);
  ...
  it("should clone account", async function () {
    ...
    expect(await account.getMode()).to.eq(0);
    expect(await account.getAdmin()).to.eq(await admin.getAddress());
    expect(await account.getOperator()).to.eq(buildOperator(deployer));
  ...
  it("should clone via eip4337 with eth transfer", async function () {
    ...
    await expect(
      ...
      .to.emit(factory, "AccountDeployed")
      .withArgs(account)
      .to.emit(accountContract, "NewOperator")
      .withArgs(hre.ethers.ZeroHash, buildOperator(deployer))
      .to.emit(accountContract, "NewAdmin")
      .withArgs(hre.ethers.ZeroAddress, await admin.getAddress());
  });
  ...
  it("should reset operator properly", async function () {
    ...
    const newOperator = buildOperator(tester1);
    // non-operator/non-admin cannot set operator
    await expect(
      account.connect(tester1).setOperator(newOperator)
    ...
    await expect(
      account.connect(deployer).setOperator(newOperator)
    ).to.be.revertedWithCustomError(account, "NotAuthorized");
    expect(await account.getMode()).to.eq(0); // admin mode
    expect(await account.getOperator()).to.eq(operator);
    ...
    const setOperatorData1 = accountIface.encodeFunctionData("setOperator", [
      newOperator,
    ]);
    await expect(
    ...
    await expect(
      ...
      .withArgs(operator, newOperator);
    expect(await account.getOperator()).to.eq(buildOperator(tester1));
    expect(await account.getMode()).to.eq(0); // admin mode

    // admin can set operator via EIP-4337 execute
    const setOperatorData2 = accountIface.encodeFunctionData("setOperator", [
      operator,
    ]);
  ...
  it("should reset admin properly", async function () {
    ...
    // will be updated since validateSignature is called
    const adminData = buildPasskeyAdminData(admin, passkey);
    setAdminData = accountIface.encodeFunctionData("setAdmin", [adminData]);
